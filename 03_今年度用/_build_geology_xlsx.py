...
def X_of_depth(depth):
    profile = [(3.0, 1.35), (2.0, 1.65), (1e9, 1.85)]
    x = 0.0
    rem = depth
    for thick, dens in profile:
        take = min(rem, thick)
        if take <= 0:
            break
        x += dens * take * 100
...
def build():
    ...
    ws["A2"] = "昨年度9班（Λ≈141 g/cm²）× 筑波台地（KEK大穂）の浅層地質モデル"
    ...
    for i, t in enumerate([
        ...
        "同じ深さでも岩盤より減衰は緩やか。それでも深さ2–3 mでMeVは地上の十数%以下まで減ると予測。",
    ...
    ws["A15"] = "主予測（MeV・地上φ=1.02E-03 基準）"
    ws["A15"].font = Font(size=13, bold=True)
    for c, h in enumerate(
        ["深さ (m)", "累積面密度 X (g/cm²)", "残存率", "予測φ (s⁻¹ cm⁻²)", "通過する主な層"], 1
    ):
        ws.cell(16, c, h)
    ...
    for i, d in enumerate([1, 2, 3, 5, 10], 17):
        x = X_of_depth(d)
        r = math.exp(-x / Lam)
        vals = [d, x, r, phi0 * r, layer_name(d)]
    ...
    for c, h in enumerate(
        ["深度区間 (m)", "地層", "密度 ρ", "区間の面密度 ΔX (g/cm²)", "累積 X (g/cm²)"], 1
    ):
        ws2.cell(12, c, h)
    style_header(ws2, 12, 5, fill="2E7D32")
    model = [
        ("0 – 3", "関東ローム", 1.35, 1.35 * 3 * 100, 405),
        ("3 – 5", "常総粘土", 1.65, 1.65 * 2 * 100, 735),
        ("5 – 10", "下総層群（砂泥）", 1.85, 1.85 * 5 * 100, 1660),
        ("10 – 20", "下総層群（砂泥）", 1.85, 1.85 * 10 * 100, 3510),
    ]
    ...
    ws2["A18"] = "注意（地質のばらつき）"
    ws2["A18"].font = Font(size=12, bold=True, color=RED)
    for i, t in enumerate([
    ...
    for i, d in enumerate(depths, 6):
        x = X_of_depth(d)
        r_mev = math.exp(-x / Lam) if d > 0 else 1.0
        r_th = math.exp(-x / Lam_th) if d > 0 else 1.0
        vals = [d, x, r_mev, phi0 * r_mev, r_th, phi0_th * r_th, layer_name(d), tips.get(d, "")]
        for c, v in enumerate(vals, 1):
    ...
    s1 = Series(Reference(ws3, min_col=3, min_row=6, max_row=17), xvals, title="MeV（Λ=141）")
    s1.graphicalProperties.line.solidFill = RED
    s1.graphicalProperties.line.width = 25000
    s1.marker = Marker(symbol="triangle", size=8)
    s1.marker.graphicalProperties.solidFill = RED
    s1.marker.graphicalProperties.line.solidFill = RED
    s2 = Series(Reference(ws3, min_col=5, min_row=6, max_row=17), xvals, title="熱（参考 Λ=203）")
    s2.graphicalProperties.line.solidFill = BLUE
    ...
    for c, h in enumerate(["深さ (m)", "MeV残存率 (%)"], 10):
        cell = ws3.cell(5, c, h)
    ...
    for i, d in enumerate([0, 1, 2, 3, 5, 10], 6):
        x = X_of_depth(d)
        r = 100.0 if d == 0 else 100.0 * math.exp(-x / Lam)
        box(ws3, i, 10, d, "0")
        box(ws3, i, 11, r, "0.0")

    bar = BarChart()
    bar.type = "col"
    bar.title = "主な深さでのMeV残存率"
    bar.height = 11
    bar.width = 13
    bar.legend = None
    ...
    for c, h in enumerate(
        ["優先", "地点イメージ", "深さ/覆土", "期待されるMeV", "ねらい", "測定時間の目安"], 1
    ):
        ws4.cell(4, c, h)
    style_header(ws4, 4, 6)
    plan = [
        ("必須", "屋外または管理棟相当の地上", "0 m（基準）", "〜1.0E-03（昨年度再現）", "基準の再現・較正", "昨年度並み"),
        ("必須", "浅い地下／ピット／B1", "約 2–3 m 相当", "地上の約 6–15%", "Λの検証（一番効く）", "地上の 5–20 倍"),
    ...
    ws4["A19"] = "2–3 m 点の MeV が、予測（残存率おおよそ 6–15%）のオーダーに入れば、昨年度ΛとKEK地質モデルが整合。"
    ws4["A20"] = "熱中性子が MeV より減りにくければ、ローム・コンクリート中の熱化／放射起源の寄与として議論できる。"
    ...
    refs = [
        ("減衰長", "昨年度9班 MeV・管理棟→白根山 Λ≈141 g/cm²（文献 130–150 と一致）"),
